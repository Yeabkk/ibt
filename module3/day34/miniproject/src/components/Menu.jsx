...
function Menu() {
  ...
  return (
    <>
      ...
      {!loading && !error && <ErrorBoundary fallback={({ reset }) => <section className="error-state" role="alert"><h2>Menu unavailable</h2><p>One dish failed to render. Your header and cart are still available.</p><button type="button" onClick={reset}>Try again</button></section>}><DishList dishes={shown} onAdd={handleAdd} /></ErrorBoundary>}
    </>
  )
}
